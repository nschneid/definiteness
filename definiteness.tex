...
\title{\draftnotice{Title, author list not final. Page limit: 8 + 2 bib (will be granted more space in CR)}Automatic Classification of Communicative Functions of Definiteness}
...
$-$ applies for labels that can {\em only} apply if the NP is new in the discourse,  
and $0$ for labels such as \llbl{Pleonastic} (where anaphoricity is not applicable because there is no discourse referent).
...
Random forests are known to be robust to sparse data and to label imbalance, both of which are challenges with the definiteness dataset.
...
will require further investigation, ideally with additional datasets.
...
Understanding definiteness may benefit applications such as coreference resolution and discourse processing, 
information extraction, and machine translation. 
For example, the anaphoric relations between a definite description and its antecedent could be realized differently 
depending on whether it is basic anaphora or extended anaphora and yet differently from non anaphoric uses of descriptions (NPs). 
A knowledge of these differences can aid coreference resolution systems in determining which entities may be linked together 
or in recognizing descriptions which are discourse-new depending on the grammaticalization they appear with. 

Similarly an understanding of definiteness in different languages can help machine translation systems. 
...
Previous work on machine translation (PBSMT) has attempted to deal with this in terms of either 
(a)~preprocessing the source language to make it look more like the target language \citep[\textit{inter alia}]{collins05,habash07,niesen00,stymne09}; 
or (b)~post-processing the machine translation output to match the target language, \cite[e.g.,][]{popovic06}. 
Attempts have also been made to use syntax on either the source or the target side or on both sides 
to get the syntactic differences between languages \citep{liu06,yamada02,zhang07}. 
...
Our work is also related to research in linguistics on the modeling of syntactic constructions 
such as dative shift and the expression of possession with ``of'' or ``'s'' in approach.  
\citet{bresnan07} used semantic features to predict syntactic constructions.   
...
Following~\citet{Hopper&Traugott, Traugott} we observe that grammaticalization is accompanied by {\em function drift}, 
resulting in multiple communicative functions for each grammatical construction. 
\ab{Hence, like \citet{bresnan07}, in this work, we used logistic regression models to explore the syntax-semantic mapping 
