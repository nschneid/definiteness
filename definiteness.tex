...
We present supervised classifiers for English that use lexical, morphological, and syntactic features 
to predict communicative functions of definiteness.   
The benefits of this work are twofold: linguistically, the classifiers' features and parameters 
...
The classifiers may therefore be useful for coreference resolution, MT,
and other mono- and cross-lingual NLP tasks. 
\end{abstract}

\section{Introduction}

Definiteness is a morphosyntactic category associated with some aspects of meaning \citep{lyons99}. While some morphosyntactic forms of definiteness are employed by all the languages, namely, demonstratives, personal pronouns and possessives, languages display a vast range of variation with respect to the form and meaning of definiteness. 
...
\item    \ilbl{Nonanaphora} $[-A,-B]$ \hfill {\bf\tiny 999}
  ...
  \item      \ilbl{Unique} $[+U]$ \hfill {\bf\tiny 287}
    \begin{itemize}
    \item        \ilbl{Unique\_Hearer\_Old} $[+F,-G,+S]$ \hfill {\bf\tiny 251}
      \begin{itemize}
      \item          \llbl{Unique\_Physical\_Copresence} $[+R]$ \hfill {\tiny 13}
      \item          \llbl{Unique\_Larger\_Situation} $[+R]$ \hfill {\tiny 237}
      \item          \llbl{Unique\_Predicative\_Identity} $[+P]$ \hfill {\tiny 1}
      \end{itemize}
    \item        \llbl{Unique\_Hearer\_New} $[-F]$ \hfill {\tiny 36}
    \end{itemize}
   \item     \ilbl{Nonunique} $[-U]$ \hfill {\bf\tiny 581}
     \begin{itemize}
     \item       \ilbl{Nonunique\_Hearer\_Old} $[+F]$ \hfill {\bf\tiny 169}
       \begin{itemize}
       \item         \llbl{Nonunique\_Physical\_Copresence} $[-G,+R,+S]$ \hfill {\tiny 39}
       \item         \llbl{Nonunique\_Larger\_Situation} $[-G,+R,+S]$ \hfill {\tiny 117}
       \item         \llbl{Nonunique\_Predicative\_Identity} $[+P]$ \hfill {\tiny 13}
       \end{itemize}
     \item       \llbl{Nonunique\_Hearer\_New\_Spec} $[-F,-G,+R,+S]$ \hfill {\tiny 231}
     \item       \llbl{Nonunique\_Nonspec} $[-G,-S]$ \hfill {\tiny 181}
     \end{itemize}
...
\item \ilbl{Anaphora} $[+A]$ \hfill {\bf\tiny 1574}
  \begin{itemize}
  \item      \ilbl{Basic\_Anaphora} $[-B,+F]$ \hfill {\bf\tiny 795}
    \begin{itemize}
    \item        \llbl{Same\_Head} \hfill {\tiny 556}
    \item        \llbl{Different\_Head} \hfill {\tiny 329}
    \end{itemize}
  \item  \ilbl{Extended\_Anaphora} $[+B]$ \hfill {\bf\tiny 779}
    \begin{itemize}
    \item        \llbl{Bridging\_Nominal} $[-G,+R,+S]$ \hfill {\tiny 43}
    \item        \llbl{Bridging\_Event} $[+R,+S]$ \hfill {\tiny 10}
    \item        \llbl{Bridging\_Restrictive\_Modifier} $[-G,+S]$ \hfill {\tiny 614}\ab{this is going in the next row, need to make everything small so it fits in the row.}
    \item        \llbl{Bridging\_Subtype\_Instance} $[-G]$ \hfill {\tiny 0}
    \item        \llbl{Bridging\_Other\_Context} $[+F]$ \hfill {\tiny 112}
    \end{itemize}
  \end{itemize}
...
This paper describes classifiers that predict communicative function labels for English NPs  
...
Our log-linear model is a form-meaning mapping, consisting of the syntactic, lexical, and morphological features 
and weights that are predictive of communicative functions. We are using this model to generate plausible hypotheses regarding the form-meaning relationship which can then be tested rigorously through controlled experiments. 
...
Our classifiers, \cref{sec:modeling}, are supervised models
with features that combine lexical and morphosyntactic information 
with prespecified attributes or groupings of the communicative function labels (such as Anaphoric, Bridging, Specific in \cref{fig:hierarchy}) to predict leaf node labels;
...
but may also have many non-prototypical communicative functions. The scheme we are adopting for this work, the annotation scheme for Communicative Functions of Definiteness (CFD), as described in \citet{bhatia14}, assumes that there may be multiple functions to definiteness. It is based on a combination of these functions and is summarized in \cref{fig:hierarchy}. It was developed by annotating texts in two languages (English and Hindi) for four different genres, namely TED talks, the presidential inaugural speech, news articles and fictional narratives, keeping in mind the communicative functions that have been associated with definiteness in the linguistic literature. 

The CFD is hierarchically organized. This hierarchical organization serves to reduce the number of decisions that an annotator needs to make
for speed and consistency. At the highest level, the distinction is made between {\bf Anaphora}, {\bf Nonanaphora} and {\bf Miscellaneous} functions of an NP (the annotatable unit). While the communicative functions {\bf Anaphora} and {\bf Nonanaphora} determine whether an entity is old or new in discourse respectively, the {\bf Miscellaneous} function is mainly assigned to various kinds of non referential NPs. Within the {\bf Anaphora} category, further distinctions are made based on communicative functions, such as {\bf Basic\_Anaphora} vs.~{\bf Extended\_Anaphora}. {\bf Basic\_Anaphora} refers to entities that have been mentioned before. {\bf Extended\_Anaphora} refers to entities that have not been mentioned themselves but are evoked by a previously mentioned entity. For example, after mentioning a wedding, {\em the bride}, {\em the groom}, and {\em the cake} 
are considered to be {\bf Extended\_Anaphora}.  Within the {\bf Nonanaphora} category, the distinction is made between a {\bf Unique} vs.~{\bf Nonunique} vs.~{\bf Generic} communicative function. The function {\bf Unique} refers to entities that become unique in a context due to various reasons. For example, {\em Obama} can safely be considered unique in a political discourse in US contemporarily. The function {\bf Nonunique} refers to entities that can possible have multiple referents which may or may not become identifiable in a speech situation. For example, {\em a little riding hood of red velvet} in \cref{fig:excerpt} could be annotated as a {\bf Nonunique} entity. The function {\bf Generic} refers to classes or types of entities rather than specific entities. For example, {\em Dinosaurs} in {\em Dinosaurs are extinct.} is a {\bf Generic}. Another important distinction CFD makes is between the communicative functions {\bf Hearer\_Old} and {\bf Hearer\_New} which refer to entities which are familiar to the hearer (e.g. if they are physically present in the speech situation) or non familiar respectively. However this distinction cuts across the two subparts of the hierarchy, {\bf Anaphora} and {\bf Nonanaphora}. While we only express {\bf Hearer\_Old} through the attribute value in the {\bf Anaphora} category, in the {\bf Nonanaphora}, there are specific labels used corresponding to this distinction, e.g. {\bf Unique\_Hearer\_Old} and {\bf Unique\_Hearer\_New}. 

There are multiple ways to organize the hierarchy based on which of these distinctions are made first and which ones are made subsequently. For example, {\citet{komen-13} has also proposed a hierarchy with similar leaf nodes, but different internal structure. Since it is possible that some natural groupings of labels are not reflected in the hierarchy used, we have also associated several attributes with each label type, viz. \uline{A}naphoric, \uline{B}ridging, \uline{F}amiliar, \uline{G}eneric, \uline{P}redicative, \uline{R}eferential, \uline{S}pecific, and \uline{U}nique. These attributes can have values of $+$, $-$, or $0$, see \cref{fig:hierarchy}. For instance, with the \uline{A}naphoric attribute, a value of $+$ applies to labels that can never mark NPs new to the discourse,
$-$ applies to labels that can {\em only} apply if the NP is new in the discourse,  
and $0$ applies to labels such as {\bf Pleonastic} (where anaphoricity is not applicable because there is no discourse referent). For further details on the scheme, see \citet{bhatia14}.
...
We also train a random forest model, which sacrifices interpretability of the learned parameters for predictive accuracy.
...
Random forests are known to be robust to sparse data and to label imbalance \citep{Chen:2004using}, both of which are challenges with the definiteness dataset.
...
to distinguish them from model features linking inputs to outputs,\footnote{See above.\finalversion{\fta{see section or subsection number?}}} 
are intended to capture the aspects of English morphosyntax that may be relevant 
to the communicative functions of definiteness. \ab{should we explain the difference between percepts and features - may be when the 1st time we mention them in section 4 intro above?- Nathan??? there we say features are lexical, morphological, syntactic characteristics of the input x, here we say these are percepts, and features link input with output.}
...
\begin{itemize}
  ...
  $|\boldsymbol{\omega}(y) \cap \boldsymbol{\omega}(y')|/9$.\finalversion{\cjd{Should these be differentially weighted based on their height in the hierarchy?}}
...
\end{itemize}
\ab{TODO: We need to enter these numbers. Chu-Cheng?}
...
The log-linear model variants are trained with an in-house implementation of supervised learning with $L_2$-regularized AdaGrad \citep{adagrad}.
...
for use in features (\cref{sec:feats}). From the parser output, the basic dependencies were used to extract the percepts used in this work. On two of the reviewers’ suggestions, to evaluate the performance of Stanford system on our data, we manually inspected the dependencies and the coreference for a subset of sentences from our corpus (using texts from TED talks and fictional narratives genres) processed using Stanford CoreNLP v.~3.3.0 and recorded the errors. We found that the Stanford parser had an accuracy of about 70\% on this subset of the corpus and coreference system had an accuracy of 75\% {\ab{TODO: enter correct numbers after we are completely done with our Gold Standard- Archna,Chu-Cheng,Jordan}. %While it is true that the errors made during preprocessing trickle down and affect the performance on the tasks at hand, e.g. classification in our case, at this stage it does not seem feasible to correct all the errors produced by the Stanford system, however this analysis indicates that there is a scope for further improvement in the classifier performance if better parsers and coreference resolvers become available. {\ab{TODO: say a little bit about what we think to what extent it could affect the overall result- Lori/Chris/Nathan/… ?}

\subsection{Results}

\begin{table*}\small\centering
   \begin{tabular}{lccc}
      \bf{Leaf-level label} & \bf{Prec.} & \bf{Recall} & \bf{F1} \\
      \midrule
Nonunique\_predicative\_identity & --- & --- & --- \\
Pleonastic & 1.0 & 0.778 & 0.875 \\
Nonunique\_generic\_nonspec & 0.087 & 0.25 & 0.129 \\
Other\_Nonreferential & 0.385 & 0.357 & 0.37 \\
Nonunique\_Larger\_situation & 0.294 & 0.25 & 0.27 \\
Bridging\_OtherContext & 0.333 & 0.063 & 0.105 \\
Same\_head & 0.413 & 0.413 & 0.413 \\
Unique\_Hearer\_new & --- & --- & --- \\
Quantified & 0.571 & 0.571 & 0.571 \\
Different\_head & 0.317 & 0.328 & 0.322 \\
Bridging\_nominal & 0.333 & 0.1 & 0.154 \\
Measure\_Nonreferential & 0.875 & 0.259 & 0.4 \\
Part\_of\_noncompositionalMWE & 0.2 & 0.167 & 0.182 \\
Bridging\_restrictiveModifier & 0.578 & 0.839 & 0.684 \\
Nonunique\_Hearer\_new\_spec & 0.358 & 0.463 & 0.404 \\
Predicative\_Nonidentity & --- & --- & --- \\
Unique\_Larger\_situation & 0.523 & 0.576 & 0.548 \\
Generic\_individualLevel & 0.143 & 0.111 & 0.125 \\
Bridging\_event & --- & --- & --- \\
Nonunique\_Physical\_copresence & --- & --- & --- \\

   \end{tabular}
   \caption{Leaf-level Precision and Recall}
   \label{tbl:leaf-level}
\end{table*}
...
Random forest classifier & 20,363 & \multicolumn{1}{c}{---} & \bf 49.7 & \bf 60.1 \\
\end{tabular}
...
the third and fourth columns give accuracies as percentages; the best in each column is bolded. \ab{Chu-Cheng will add precision and recall for the exact match. We can’t do that for soft-match.    Also Chu-Cheng will add another table where we have accuracies corresponding to each leaf label.}}
\label{tbl:results}
\end{table*}

Measurements of overall classification performance appear in \cref{tbl:results}. 
While far from perfect, our classifiers achieve promising accuracy levels 
given the small size of the training data and the number of labels in the annotation scheme.
The random forest classifier is the most accurate, likely due to the robustness of that technique 
under conditions where the data are small and the frequencies of individual labels are imbalanced.

Among the log-linear models, the most successful is the richest model, which combines the fine-grained 
communicative function labels with higher-level attributes of those labels. 
...
\multicolumn{1}{c}{\bf $+$Specific} & \multicolumn{1}{c}{\bf $-$Specific} \\
\begin{verbatim}
