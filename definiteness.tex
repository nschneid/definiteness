...
Exploiting the higher level semantic attributes, our log-linear classifier compares favorably to the random forest classifier in Soft Match accuracy. With the attributes, we should also be able to predict intermediate level CFDs and design a cost function that allows us to directly optimize the classifier for Soft Match. Preliminary results do show improvement, we plan to explore this area further. This work has focused on English, but in future work we plan to build similar models for other languages---including 
