...
Most percepts of the model are local around the head, so when several highly weighted percepts may be fired because of what is usually regarded as a single construction. This is illustrated in Figure \ref{featureweights}: the model `ses' 3 highly weighted percepts, namely the POS of the first dependent, the head's left neighbor's POS, and the governor's right neighbor's POS to \emph{describe} a possessive pronoun adjecent to the noun it depends on. 
...
Other attempts have also been made to capture,  using classifiers, (propositional as well as non propositional) aspects of meaning that have been grammaticalized, e.g. see \cite{reichart10} for tense sense disambiguation, \cite{prabhakaran12}. 
