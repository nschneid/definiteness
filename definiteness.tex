...
Definiteness expresses a constellation of semantic, pragmatic, and discourse properties---the communicative function---of an NP. We present a supervised classifier for English NPs that uses lexical, morphological, and syntactic features to 
predict an NP's communicative functions in terms of a language-universal classification scheme. Our classifiers establish strong baselines for future work in this neglected area of computational semantic analysis. In addition, the analysis of the features and learned parameters 
provides insight into the grammaticalization of definiteness in English, 
not all of which are obvious a priori.
...
According to \cite{lyons99}, definiteness is a morphosyntactic category expressing aspects of meaning of Noun Phrases (NPs), such as identifiability (whether a referent to an NP is identifiable by the discourse participants). While some morphosyntactic forms of definiteness are employed by all languages---namely, demonstratives, personal pronouns and possessives---languages display a vast range of variation with respect to the form and meaning of definiteness. 
...
in the mapping between semantic, pragmatic, and discourse--related functions of NPs and the (in)definites expressing these functions.   
We refer to these as {\em communicative functions} of (in)definiteness, following \cite{bhatia14}.  
...
to distinguish them from model features linking inputs to outputs,\footnote{See above. \fta{see section or subsection number?} \ab {section 4 intro???}} 
...
\begin{table*}\small\centering
   \begin{tabular}{lccc}
      \bf{Leaf label} & \bf{Prec.} & \bf{Recall} & \bf{F1} \\
      \midrule
...
Nonunique\_Physical\_copresence & --- & --- & --- \\
   ...
   \end{tabular}
   \caption{Leaf Precision and Recall \ab{discuss briefly in Results section}}
   \label{tbl:leaf}
\end{table*}
...
& 12.1 & 52.3
\\
Log-linear classifier, attributes only & 473,064 & 100 & 38.7 & 81.9 \\
Log-linear classifier, labels only & 413,931 & 100 & 40.8 & 77.7  \\
Full log-linear classifier (labels + attributes) & 926,417 & 100 & 43.7 & 82.5 \\
Random forest classifier & 20,363 & \multicolumn{1}{c}{---} & \bf 49.7 & \bf 81.0 \\
\end{tabular}
...
The Random forest classfier is the most accurate in Exact Match, like due to the robustness under
conditions where the data are small and the frequencies of individual labels are imbalanced. However
our attribute-aware log-linear models have the best accuracy under the SoftMatch measure.
