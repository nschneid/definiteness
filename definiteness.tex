...
and that NPs with comparative adjectives (\texttt{JJR}) are especially likely to be nonspecific, as in \cref{fig:weights}. For another example in \cref{fig:weights}, \emph{Num. of dependents, dependent's POS: $1$,PRP\$} has higher weight than, say \emph{Num. of dependents, dependent's POS: $2$,PRP\$}. This observation can lead us to formulate a hypothesis that in English the NPs which have possessive pronouns immediately preceding the head are more likely to be specific than the NPs which have intervening words between the possessive pronoun and the head. Similarly, looking at another example in \cref{fig:featureweights}, the following two percepts fire for the NP “the United States of America”, viz. \emph{last dependent’s POS: NNP}, and \emph{first dependent’s lemma: the}, we may hypothesize that these percepts are indicators of specificity. Thus this methodology allows us to generate hypotheses, however, each of these hypotheses may not hold true under further investigation. This is due to the fact that several factors affect the feature weights the classifier generates: either these weights indeed reflect the inherent association with specificity, or they may result from the frequency for that pattern observed in our corpus. Whether these are general trends, or just an artifact of the sentences that happened to be in the training data, will require further investigation, ideally with additional datasets and more rigorous hypothesis testing.
...
Exploiting the higher level semantic attributes, our log-linear classifier compares favorably to the Random forest classifier in SoftMatch accuracy. With the attributes, we should also be able to predict intermediate level CFDs and design a cost function that allows us to directly optimize the classifier for SoftMatch. Preliminary results do show improvement, we plan to explore this area further. This work has focused on English, but in future work we plan to build similar models for other languages---including 
