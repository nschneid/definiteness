...
\Cref{tbl:leaf} reports the precision and recall of each leaf label predicted. 
Certain leaf labels are found to be easier for the classifier to predict: 
e.g., the communicative function label Pleonastic has a high $F_1$ score. 
This is expected as the Ploenastic CFD for English is quite regular and captured by the \texttt{EX} part-of-speech tag. 
%The percept ``POS is EX'' fires consistently for NPs which are gold-annotated with Pleonastic CFD label in English. 
The classifier predictions certain CFD labels, 
such as Bridging\_Event, Bridging\_Nominal and Nonunique\_Nonspecific, to be more difficult due to 
...
Bridging\_Other\_Context was hard to predict as this was a category which referred not to the entities previously mentioned 
but to the whole speech event. There seem to be no clear morphosyntactic cues associated with this CFD, 
so to train a classifier to predict this category label, we would to model more complex semantic and discourse information. 
This is also applicable to the confusion between the Same\_Head and Different\_Head, 
...
for the \mbox{$+$ and $-$ values} of the Specific attribute. 
...
here the model thinks that objects of ``from'' are especially likely to be specific---see \cref{fig:weights,fig:featureweights}---and 
that NPs with comparative adjectives (\texttt{JJR}) are especially likely to be nonspecific (\cref{fig:weights}). 
From \cref{fig:weights} we also know that \emph{Num. of dependents, dependent's POS: $1$,PRP\$} has a higher weight than, 
say, \emph{Num. of dependents, dependent's POS: $2$,PRP\$}. 
...
% Similarly, looking at another example in \cref{fig:featureweights}, the following two percepts fire 
% for the NP “the United States of America”, viz. \emph{last dependent’s POS: NNP}, and \emph{first dependent’s lemma: the}, 
% we may hypothesize that these percepts are indicators of specificity. 
Because frequency and other factors affect the feature weights learned by the classifier, however, 
these differences in weights may or may not reflect an inherent association with specificity.
...
\end{tabular}
\caption{Sentences from our corpus to illustrate Percepts fired for \emph{the Gold NPs} and the corresponding CFD annotations.}
...
distinguish between generic and specific NPs. \Citet{hendrickx11} investigated the extent to which a coreference resolution system can resolve the bridging relations.  
