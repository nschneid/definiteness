...
According to \cite{lyons99}, definiteness is a morphosyntactic category expressing aspects of meaning of Noun Phrases (NPs), such as identifiability (whether a referent to an NP is identifiable by the discourse participants or not). While some morphosyntactic forms of definiteness are employed by all languages---namely, demonstratives, personal pronouns and possessives---languages display a vast range of variation with respect to the form and meaning of definiteness. 
...
CFD is hierarchically organized. This hierarchical organization serves to reduce the number of decisions that an annotator needs to make
...
\begin{itemize}
  ...
  $|\boldsymbol{\omega}(y) \cap \boldsymbol{\omega}(y’)|/8$.\finalversion{\cjd{Should these be differentially weighted based on their height in the hierarchy?}}
...
Gold NP boundaries are assumed throughout our experiments. Thus, for training as well as testing, we extract percepts of only the gold NPs and do not consider the other possible NPs inferred by the parser. For instance, to find the head of a gold NP, we assume the gold boundary and look for candidates within this boundary which have an outbound dependency. If there is a conflict between the gold boundaries and the parser generated boundaries or multiple candidates satisfying the above condition are found, then we select the candidate from within the gold boundary which is linearly closest to its outbound head. 
...
for use in features (\cref{sec:feats}). From the parser output, the basic dependencies were used to extract the percepts used in this work. On two of the reviewers’ suggestions, to evaluate the performance of Stanford system on our data, we manually inspected the dependencies for a subset of sentences from our corpus (using texts from TED talks and fictional narratives genres) processed using Stanford CoreNLP v.~3.3.0 and recorded the errors. We found that about 70\% of the sentences had all correct dependencies, and only about 0.04\% of the total dependencies were incorrect for our data.
